<div class="card col-sm-12">
    <h3 class="card-header primary-color white-text mt-3 text-center">Level one</h3>
    ...
    <div class="card-body col-sm-7 m-auto answer_form_surface_view answer_form">
            <?php
            foreach ($questions as $question) {

                $id = $this->session->get('user_id');
                $queId = $question->question_id;
                $sql = "SELECT * FROM surface_measure_answers WHERE student_id = $id AND question_id = $queId";
                $answers = \Models\SurfaceMeasureAnswersModel::query($sql);
                ...
                <form action="<?= $this->route->baseUrl() . 'exams/addMeasure'?>" method="post" class="d-none form_<?= $question->question_id?>">
                    ...
                    <input type="hidden" name="model" value="SurfaceMeasureAnswersModel">
                    <input type="hidden" name="table" value="surface_measure_answers">
            ...
            <?php } ?>
        <a href="<?= $this->route->baseUrl() . 'exams/deep'?>" class="btn btn-success text-light text-uppercase mb-5 d-none save_surface_answer">Save answers</a>
    </div>
