    ...
    <div class="card-body col-sm-10 m-auto answer_form">
        ...
        foreach ($questions as $question) {
            ...
            <form action="<?= $this->route->baseUrl() . 'measure/examAdd'?>" method="post" id="form_<?= $question->question_id?>" class="form_<?= $question->question_id?> mb-5 d-none">
                <input type="hidden" name="model" value="AnswersDeepAfterModel">
                <input type="hidden" name="table" value="answers_after_deep">
                ...
                foreach ($choices as $choice) {
                    ...
                    $sql = "SELECT * FROM answers_after_deep WHERE question_id = $queId AND student_id = $sId";
        ...
        <?php } ?>
        <a href="<?= $this->route->baseUrl() . 'measure/deepAfter'?>" class="btn btn-success text-light text-uppercase mb-5 d-none save_surface_answer">Save answers</a>
    </div>
