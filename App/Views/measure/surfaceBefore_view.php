...
<div class="card col-sm-12">
    <h3 class="card-header primary-color white-text mt-3 text-center">Exam before</h3>
    <style>
        .progressbar {
            width: 80%;
            margin: 25px auto;
    ...
    <div class="col-lg-7 m-auto d-flex justify-content-center align-items-center">
        ...
        <div data-href="<?= $this->route->baseUrl() . 'SurfaceTrack'?>" id="counter" class="text-success"></div>
    </div>
    ...
    <div class="card-body col-sm-10 m-auto answer_form">
        ...
        foreach ($questions as $question) {
            ...
            <form action="<?= $this->route->baseUrl() . 'measure/examAdd'?>" method="post" id="form_<?= $question->question_id?>" class="form_<?= $question->question_id?> mb-5 d-none">
                <input type="hidden" name="model" value="AnswersSurfaceBeforeModel">
                <input type="hidden" name="table" value="answers_before_surface">
                ...
                foreach ($choices as $choice) {
                    ...
                    $sql = "SELECT * FROM answers_before_surface WHERE question_id = $queId AND student_id = $sId";
        ...
        <?php } ?>
        <a href="<?= $this->route->baseUrl() . 'SurfaceTrack'?>" class="btn btn-success text-light text-uppercase mb-5 d-none save_surface_answer">Save answers</a>
    </div>
